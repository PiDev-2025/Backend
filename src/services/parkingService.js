...
const createParking = async (req, res) => {
  try {
    ...
    if (typeof pricing !== 'object' || 
        typeof pricing.hourly !== 'number') {
      return res.status(400).json({ message: 'Les informations de tarification horaire doivent être des nombres' });
    }

    // Vérification des types de véhicules
    if (!Array.isArray(vehicleTypes) || vehicleTypes.length === 0) {
      return res.status(400).json({ message: "Les types de véhicules sont requis" });
    }
    
    // Vérification des fonctionnalités si présentes
    if (features && !Array.isArray(features)) {
      return res.status(400).json({ message: "Les fonctionnalités doivent être un tableau" });
    }
...
const updateParking = async (req, res) => {
  try {
    ...
    if (!name || !position || !totalSpots || !availableSpots || !pricing || !vehicleTypes) {
      return res.status(400).json({ message: "Tous les champs obligatoires sont requis pour la mise à jour" });
    }

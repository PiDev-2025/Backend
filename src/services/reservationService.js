...
const checkAvailability = async (req, res) => {
  try {
    ...
    if (!isValidObjectId(parkingId)) {
      return res
        .status(400)
        .json({ success: false, message: "ID de parking invalide" });
    }

    if (!spotId || !spotId.startsWith("parking-spot-")) {
      return res
        .status(400)
        .json({ success: false, message: "ID de place invalide" });
    }
    ...
    if (isNaN(start.getTime()) || isNaN(end.getTime())) {
      return res
        .status(400)
        .json({ success: false, message: "Dates invalides" });
    }
...
const deleteReservation = async (req, res) => {
  try {
    ...
    if (parking) {
      ...
      console.warn(
        "⚠️ Parking non trouvé pour la réservation:",
